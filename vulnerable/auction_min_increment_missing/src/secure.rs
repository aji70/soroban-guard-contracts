...
impl SecureAuction {
    ...
    pub fn bid(env: Env, bidder: Address, amount: i128) {
        ...
        let min_increment: i128 = env.storage().persistent().get(&DataKey::MinIncrement).expect("not initialized");
        // ✅ Enforce minimum increment.
        let required = high_bid.checked_add(min_increment).expect("overflow");
        if amount < required { panic!("bid does not meet minimum increment"); }
