...
impl VulnerableAuction {
    ...
    /// VULNERABLE: settles without checking end ledger.
    ...
    pub fn high_bid(env: Env) -> i128 {
        env.storage().persistent().get(&DataKey::HighBid).unwrap_or(0)
    }
    ...
    pub fn is_closed(env: Env) -> bool {
        env.storage().persistent().get(&DataKey::Closed).unwrap_or(false)
    }
}
...
mod tests {
    ...
    fn test_secure_rejects_close_before_end_ledger() {
        ...
        env.mock_all_auths();
        client.initialize(&500u32);
        let bidder = Address::generate(&env);
        client.bid(&bidder, &100);
        let result = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| { client.close(); }));
